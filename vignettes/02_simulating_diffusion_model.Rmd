...
source("../R/02_diffusion_simulator.R")
```
...
if(nrow(valid_ddm_rt_data) > 0) {
  # Plot for total RT
  p_rt_total <- ggplot(valid_ddm_rt_data, aes(x = rt, fill = factor(choice))) +
    ...
    labs(
      title = "DDM Total RT Distributions",
      x = "Reaction Time (s)",
      y = "Frequency",
      fill = "Choice"
    ) +
...
n_paths_to_plot <- 10 # Number of example paths
all_paths_data <- vector("list", n_paths_to_plot) # To store data for each path

set.seed(303) # For reproducibility of these specific paths
