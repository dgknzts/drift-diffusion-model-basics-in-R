...
verify_analytical_solution <- function(n_sims = 1000) {
  ...
  p1 <- ggplot(results, aes(x = p_analytical, y = p_simulated)) +
    ...
    labs(title = sprintf("Analytical vs Simulated Choice Probabilities (n=%d per condition)", n_sims),
  ...
  p2 <- ggplot(results, aes(x = interaction(v, z_ratio), y = difference)) +
    ...
    geom_point(size = 3) +
    geom_errorbar(aes(ymin = -2*se_simulated, ymax = 2*se_simulated), width = 0.3) +
...
verify_analytical_solution(n_sims = 500)
```
