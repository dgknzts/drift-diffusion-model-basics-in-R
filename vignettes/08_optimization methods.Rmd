---
title: "Optimization Methods in DDM Fitting: Finding the Best Parameters"
...
output:
  html_document:
    ...
    df_print: kable
vignette: >
  %\VignetteIndexEntry{9. Optimization in DDM Fitting}
...
candidate_mean_vs <- seq(0.05, 0.45, by = 0.005) # A range of v values to test
...
for (v_test in candidate_mean_vs) {
  ...
  current_params_manual <- fixed_params_manual
  current_params_manual$mean_v <- v_test # Set the candidate v

  set.seed(666) # Use same seed for all candidate sims for fair comparison of v effect
  ...
  if (nrow(sim_data_candidate) > 0 && any(!is.na(sim_data_candidate$choice))) {
    ...
    target_subset <- target_stats_manual[c("p_correct", "rt_correct_q50", "rt_error_q50")]
...
cat(paste("\n(True mean_v was:", true_manual_mean_v, ")\n"))
```
...
if(nrow(search_results) > 0 && any(is.finite(search_results$error_score))) {
  ggplot(search_results, aes(x = candidate_v, y = error_score)) +
    ...
    geom_vline(xintercept = true_manual_mean_v, linetype = "dashed", color = "red") +
    ...
    annotate("text", x = true_manual_mean_v, y = max(search_results$error_score, na.rm=T)*0.9, 
    ...
    labs(
      ...
      subtitle = paste("True mean_v =", true_manual_mean_v, "; Best found =", best_v_manual),
      x = "Candidate mean_v",
      y = "Error Score (SSD)"
    ) +
    theme_minimal()
...
n_target_trials_for_optim <- 5000
...
We define which parameters optim should try to find, their initial guesses, and their bounds. We also tell our ddm_objective_function (modified in R/04_ddm_fitting_utils.R) to enforce mean_z = a/2.
...
upper_bounds_for_optim <- c(mean_v = 0.8,  a = 2.5, mean_ter = 0.7)
