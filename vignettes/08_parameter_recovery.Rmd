...
## Step 2.5: Diagnostic Check

Before running the full optimization, let's test our objective function with the true parameters to ensure it's working correctly:

```{r diagnostic_check}
# Run diagnostic to check if objective function works properly
diagnostic_results <- diagnose_ddm_fitting(
  true_params = true_params_adv,
  target_binned_props = target_binned_props_adv,
  objective_fn_name = "ddm_binned_likelihood_objective",
  n_sim_per_eval = 2000,
  fixed_params = list(dt = fixed_dt_adv, correct_choice_value = 1, error_choice_value = 0),
  rt_bins = rt_bins_for_likelihood,
  constrain_z_to_a_div_2 = TRUE,
  param_names_optim = c("mean_v", "a", "s", "mean_ter", "sv", "sz", "st0")
)
```

If the diagnostic shows that the objective function value with true parameters is reasonable and perturbed parameters give worse (higher) values, then the objective function is working correctly.
...
initial_guesses_means_adv <- c(
  mean_v   = 0.18, a = 0.6,  s = 0.2, mean_ter = 0.1,  # Closer to true values
  sv = 0.15, sz = 0.06, st0 = 0.05
)
initial_guesses_sds_adv <- c( # SDs for sampling initial guesses
  mean_v   = 0.05, a = 0.2,  s = 0.05, mean_ter = 0.03,  # Reduced SDs for tighter search
  sv = 0.05, sz = 0.02, st0 = 0.02
)

# Bounds for parameters (must match param_names_to_optimize_adv)
lower_bounds_adv <- c(
  mean_v = -0.3, a = 0.2,  s = 0.05, mean_ter = 0.01,  # Tightened some bounds
  sv = 0,      sz = 0,   st0 = 0
)
upper_bounds_adv <- c(
  mean_v = 0.5,  a = 1.5,  s = 0.5, mean_ter = 0.15,   # Better aligned with true values
  sv = 0.4,   sz = 0.2,  st0 = 0.1 # More reasonable upper bounds
)
# Ensure upper bound for sz is reasonable if 'a' is small
upper_bounds_adv["sz"] <- min(upper_bounds_adv["sz"], lower_bounds_adv["a"]*0.8)
...
n_optim_starts <- 5    # Increased from 3 for better coverage
optim_n_sim_adv <- 2000 # Increased from 1000 for more stable likelihood estimates
optim_maxit_adv <- 100   # Increased from 50 for better convergence
