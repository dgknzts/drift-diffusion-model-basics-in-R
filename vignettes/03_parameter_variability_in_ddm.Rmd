---
title: "3. Exploring Across-Trial Parameter Variability in the DDM"
...
output:
  html_document:
    ...
    df_print: kable
vignette: >
  %\VignetteIndexEntry{3. Parameter Variability in DDM}
...
# Assuming your files are named as discussed:
...
While this vignette has shown static examples of how parameter variability impacts DDM predictions, you can explore these effects dynamically using the Shiny application included in this project (`app.R`).

The app allows you to: \* Modify all key DDM parameters (means and variabilities) using sliders and numeric inputs. \* Instantly re-simulate and visualize the resulting DDM paths, RT distributions, and sampled parameter distributions.

This provides an excellent hands-on way to build intuition about the model.

**To run the Shiny app:** 1. Open the `DDM_Basics_R.Rproj` file in RStudio. 2. Ensure the necessary packages (`shiny`, `ggplot2`, `dplyr`, `patchwork`) are installed. 3. Open `app.R` and click "Run App".
