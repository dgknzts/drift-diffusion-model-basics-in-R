...
While this vignette has shown static examples of how parameter variability impacts DDM predictions, you can explore these effects dynamically using the Shiny demonstration included in this project in the `local-app` directory.

The demo allows you to: 
* Modify all key DDM parameters (means and variabilities) using sliders and numeric inputs. 
* Instantly re-simulate and visualize the resulting DDM paths, RT distributions, and sampled parameter distributions.

This provides an excellent hands-on way to build intuition about the model.

**To run the Shiny demo:** 
1. Open the `DDM_Basics_R.Rproj` file in RStudio. 
2. Ensure the necessary packages (`shiny`, `ggplot2`, `dplyr`, `patchwork`) are installed. 
3. Run the command `shiny::runApp("local-app")` in the R console.
