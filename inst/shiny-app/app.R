...
# Adjust paths as if app.R is in the root of your DDM_Basics_R project
...
server <- function(input, output, session) {
  ...
  output$choice_prop_shiny <- renderPrint({
    df_full <- full_sim_df() # Get all simulated data
    req(df_full)
    if (nrow(df_full) == 0) return("No simulation data.")
    
    # Use the plot_max_time_shiny input for filtering, if provided and valid
    current_max_time <- input$plot_max_time_shiny
    df_for_summary <- df_full # Start with all data
    
    if (!is.na(current_max_time) && is.finite(current_max_time) && current_max_time > 0) {
      df_for_summary <- df_full %>% filter(rt <= current_max_time | is.na(rt)) # Keep NAs to count timeouts correctly if desired
    }
    
    if (nrow(df_for_summary) == 0 || all(is.na(df_for_summary$choice))) {
      return("No valid choices to summarize within the specified time limit (if any).")
    }
    
    cat("Choice Proportions (potentially filtered by plot_max_time):\n")
    print(prop.table(table(df_for_summary$choice, useNA = "ifany")))
  })
  
  output$rt_summary_shiny <- renderTable({
    df_full <- full_sim_df() # Get all simulated data
    req(df_full)
    if (nrow(df_full) == 0) return(data.frame(Message="No simulation data."))
    
    # Use the plot_max_time_shiny input for filtering
    current_max_time <- input$plot_max_time_shiny
    df_for_summary <- df_full # Start with all data
    
    if (!is.na(current_max_time) && is.finite(current_max_time) && current_max_time > 0) {
      df_for_summary <- df_full %>% filter(rt <= current_max_time | is.na(rt)) 
    }
    
    if (nrow(df_for_summary) == 0 || all(is.na(df_for_summary$rt[which(!is.na(df_for_summary$choice))]))) { # Check if any non-NA choices have RTs
      return(data.frame(Message="No valid RTs to summarize within the specified time limit (if any)."))
    }
    
    summary_data <- df_for_summary %>%
      filter(!is.na(choice)) %>% # Now, only summarize for actual choices from the (potentially) time-filtered data
      group_by(Choice = factor(choice, levels=c(0,1), labels=c("Lower (0)", "Upper (1)"))) %>%
      summarise(
        N = n(), # This N now reflects choices with RT <= max_time_to_plot (if set)
        Mean_RT = round(mean(rt, na.rm = TRUE), 3),
        Median_RT = round(median(rt, na.rm = TRUE), 3),
        SD_RT = round(sd(rt, na.rm = TRUE), 3),
        Min_RT = if(all(is.na(rt))) NA else round(min(rt, na.rm = TRUE), 3),
        Max_RT = if(all(is.na(rt))) NA else round(max(rt, na.rm = TRUE), 3),
        .groups = "drop"
      )
    
    # If no rows after filtering, return an informative message
    if(nrow(summary_data) == 0) {
      return(data.frame(Message="No choices made within the specified time limit to summarize."))
    }
